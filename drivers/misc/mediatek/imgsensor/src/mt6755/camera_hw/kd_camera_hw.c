...
#define DEBUG_CAMERA_HW_K 0
#define CONTROL_AF_POWER 0

#if DEBUG_CAMERA_HW_K
...
PowerUp PowerOnList = {
	{
	 ...
	 {SENSOR_DRVNAME_OV8858_MIPI_RAW,
	  ...
	  {
	   {SensorMCLK, Vol_High, 0},
	   {RST, Vol_Low, 0},		//For Ar1337  RST >> PDN
	   {AVDD, Vol_2800, 0},
	   {DOVDD, Vol_1800, 0},
	   {DVDD, Vol_1200, 10},
	   {RST, Vol_Low, 0},
...
struct pinctrl_state *cam0_ldo_vcama_h = NULL;/* for AVDD */
struct pinctrl_state *cam0_ldo_vcama_l = NULL;
struct pinctrl_state *cam1_ldo_vcama_h = NULL;/* for AVDD */
struct pinctrl_state *cam1_ldo_vcama_l = NULL;
struct pinctrl_state *cam2_ldo_vcama_h = NULL;/* for AVDD */
struct pinctrl_state *cam2_ldo_vcama_l = NULL;
...
//struct pinctrl_state *cam1_ldo_vcama_h = NULL;/* for SUB_AVDD */
//struct pinctrl_state *cam1_ldo_vcama_l = NULL;
...
int mtkcam_gpio_init(struct platform_device *pdev)
{
	...
	/*externel LDO enable */
	cam0_ldo_vcama_h = pinctrl_lookup_state(camctrl, "cam0_ldo_vcama_1");
	if (IS_ERR(cam0_ldo_vcama_h)) {
		ret = PTR_ERR(cam0_ldo_vcama_h);
		PK_DBG("%s : pinctrl err, cam0_ldo_vcama_h\n", __func__);
	}

	cam0_ldo_vcama_l = pinctrl_lookup_state(camctrl, "cam0_ldo_vcama_0");
	if (IS_ERR(cam0_ldo_vcama_l)) {
		ret = PTR_ERR(cam0_ldo_vcama_l);
		PK_DBG("%s : pinctrl err, cam0_ldo_vcama_l\n", __func__);
	}
	...
	cam2_ldo_vcama_h = pinctrl_lookup_state(camctrl, "cam2_ldo_vcama_1");
	if (IS_ERR(cam2_ldo_vcama_h)) {
		ret = PTR_ERR(cam2_ldo_vcama_h);
		PK_DBG("%s : pinctrl err, cam_ldo_vcama_h\n", __func__);
	}
	cam2_ldo_vcama_l = pinctrl_lookup_state(camctrl, "cam2_ldo_vcama_0");
	if (IS_ERR(cam2_ldo_vcama_l)) {
		ret = PTR_ERR(cam2_ldo_vcama_l);
...
int mtkcam_gpio_set(int PinIdx, int PwrType, int Val)
{
	...
	case AVDD_EN:
		if (PinIdx == 0) {
			if (Val == 0 && !IS_ERR(cam0_ldo_vcama_l))
				pinctrl_select_state(camctrl, cam0_ldo_vcama_l);
			else if (Val == 1 && !IS_ERR(cam0_ldo_vcama_h))
				pinctrl_select_state(camctrl, cam0_ldo_vcama_h);
			else
				PK_ERR("%s : pinctrl err, PinIdx %d, Val %d, AVDD\n", __func__,PinIdx ,Val);
		} else if (PinIdx == 1) {
			if (Val == 0 && !IS_ERR(cam1_ldo_vcama_l))
				pinctrl_select_state(camctrl, cam1_ldo_vcama_l);
			else if (Val == 1 && !IS_ERR(cam1_ldo_vcama_h))
				pinctrl_select_state(camctrl, cam1_ldo_vcama_h);
			else
				PK_ERR("%s : pinctrl err, PinIdx %d, Val %d, AVDD\n", __func__,PinIdx ,Val);
		} else {
			if (Val == 0 && !IS_ERR(cam2_ldo_vcama_l))
				pinctrl_select_state(camctrl, cam2_ldo_vcama_l);
			else if (Val == 1 && !IS_ERR(cam2_ldo_vcama_h))
				pinctrl_select_state(camctrl, cam2_ldo_vcama_h);
			else
				PK_ERR("%s : pinctrl err, PinIdx %d, Val %d, AVDD\n", __func__,PinIdx ,Val);
		}
		break;
...
BOOL hwpoweron(PowerInformation pwInfo, char *mode_name)
{
	...
	}else if(pwInfo.PowerType == AVDD_EN){
	//	mtkcam_gpio_set(pinSetIdx, AVDD_EN, pinSet[pinSetIdx][IDX_PS_AVDD+ IDX_PS_OFF]);
		if (pwInfo.Voltage == Vol_High) {
			if (mtkcam_gpio_set(pinSetIdx, AVDD_EN, Vol_High)) {
				PK_ERR("[CAMERA SENSOR] set gpio failed!!\n");
			}
		} else {
			if (mtkcam_gpio_set(pinSetIdx, AVDD_EN, 0)) {
				PK_ERR("[CAMERA SENSOR] set gpio failed!!\n");
			}
		}
		}
...
BOOL hwpowerdown(PowerInformation pwInfo, char *mode_name)
{
	if (pwInfo.PowerType == AVDD) {
		...
		if (pwInfo.Voltage == Vol_High) {
			if (mtkcam_gpio_set(pinSetIdx, AVDD_EN, pinSet[pinSetIdx][IDX_PS_AVDD + IDX_PS_ON])) {
				PK_ERR("[CAMERA SENSOR] set gpio failed!!\n");
			}
		} else {
			if (mtkcam_gpio_set(pinSetIdx, AVDD_EN, pinSet[pinSetIdx][IDX_PS_AVDD + IDX_PS_OFF])) {
				PK_ERR("[CAMERA SENSOR] set gpio failed!!\n");
			}
		}
	}
